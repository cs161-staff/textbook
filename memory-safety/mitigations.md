...
that prevent memory errors from occurring. In an ideal world, everyone would
...
if still need to execute arbitrary code. Return-oriented programming is a
technique overwrites a chain of return addresses starting at the RIP in order to
execute a series of "ROP gadgets" which are equivalent to the desired malicious
code. For example, imagine we want to add 4 to the value currently in the EDX
...
support $$2^{64}$$ bytes, or 16 exabytes of memory, but we are a long way off
from having a machine with this much memory. A modern CPU might support a 16
terabyte address space, which means 44 bits are needed to address all of memory.
This still leaves 20 unused bits in every address and pointer.

Consider using these unused bits to store a secret like the stack canary. Any
time we need to store an address on the stack, the CPU first replaces the 20
unused bits with some secret value before pushing the value on the stack. When
the CPU reads an address off the stack, it will check that the secret value is
unchanged. If the secret is unchanged, then the CPU replaces the secret with the
original unused bits and uses the address normally. However, if the secret has
...
unused bytes with a secret value. For example, if the secret value is `0xABCDE`,
...
the stack and check that the secret `0xABCDE` is unchanged. If the secret is
...
Now, an attacker trying to overwrite the rip would need to know the secret in
order to overwrite the rip with the address of some attacker shellcode. If the
attacker overwrites the secret with an incorrect value, the CPU will detect this
and crash the program.

We can strengthen this defense even further. Since it is the CPU's job to add
and check the secret, we can ask the CPU to use a different secret for every
pointer stored on the stack. However, we don't want to store all these secrets
on the CPU, so we'll use some special math to help us generate secure secrets on
the fly.
...
and outputs a secret value by performing some operation on these two inputs.
...
Now, instead of using the same secret value for every address, we can generate a
different secret value for each address we store in memory. Every time an
...
address to re-generate the secret value, and checks that the generated value
...
Using a different secret value for every address makes this defense extremely
...
the secret value untouched, but because they've changed the address, the old
...
secret values doesn't help the attacker generate a valid secret value for their
chosen malicious address.

Recall that stack canaries can be implemented entirely in software: the compiler
adds some code at the start of the function to insert a canary, and then adds
some code at the end of the function to check the canary. Pointer
authentication, on the other hand, requires hardware implementation. The CPU
must be able to add secrets when writing to memory, and check secrets when
reading from memory.

Many implementations of pointer authentication use a special CPU instruction to
transform a raw pointer into an authenticated pointer (an address with the
secret attached). The CPU stores the secret key so that it is inaccessible to
the rest of the program. Another special CPU instruction is used to check that
an authenticated pointer hasn't been tampered with by an attacker. This
instruction crashes the program if it detects that the pointer has been changed.
A simple way to crash the program when the pointer has changed is to leave the
pointer in its corrupted state, so that when the program tries to dereference
the pointer, the memory management system sees an invalid address and triggers
an exception.

Today, ARM 8.3 supports pointer authentication. Notably, the latest Apple chips,
starting with the A12 and including the new M1, support pointer authentication
natively.
