...
assembly, which is different from RISC-V assembly taught in 61C.
...
To distinguish between binary and hexadecimal strings, we put `0b` before binary
...
2. The _assembler_ translates the assembly instructions from the compiler into
   ...
   translate assembly instructions into raw bits in 61C. This is what the assembler
   does.
...
![1-dimensional address space](images/1d-address-space.png)

However, this is hard to read, so we can rearrange the memory into a grid. In
...
![2-dimensional address space](images/2d-address-space.png)
...
When a program is being run, the address space is divided into four sections.
From lowest address to highest address, they are:

- The _code_ section contains the executable instructions of the program. Recall
  that the assembler and linker output raw bytes that can be interpreted as
  machine code. These bytes are stored in the code section.
...
- The _heap_ stores dynamically allocated data. When you call `malloc` in C,
  ...
  `free`. The heap starts at lower addresses and "grows up" to higher addresses as
...
![Memory Sections](images/memory-sections.png)
...
![Little-endian word format](images/little-endian.png)
...
In addition to the $2^{32}$ bytes of memory in the address space, there are also
_registers_, which store memory directly on the CPU. Each register can store one
word (4 bytes). Unlike memory, registers do not have addresses. Instead, we
refer to registers using names. There are three special x86 registers that are
relevant for these notes:

- _eip_ is the _instruction pointer_. The address of the machine instruction
  currently being executed is stored in this register. In RISC-V, this register
  is called PC (program counter).
- _ebp_ is the _base pointer_. The address of the top of the current stack frame
  is stored in this register. In RISC systems, this register is called `FP`
  (frame pointer)[^4].
- _esp_ is the _stack pointer_. The address of the bottom of the stack is stored
  in this register. In RISC-V, this register is called `SP` (stack pointer).
...
space on the stack by decrementing esp. Then we store the value in the newly
allocated space. The x86 `push` instruction does both of these steps to add a
value to the stack.

![Before and after of pushing an item onto the stack](images/push.png)
...
![Before and after of popping an item off the stack](images/pop.png)
...
argument 1 pushed below argument 2](images/stack1.png)
...
![Next stack diagram, with the eip moved to the code for foo](images/stack3.png)
...
![Next stack diagram, with the ebp moved to the esp](images/stack5.png)
...
![Next stack diagram, with the esp down by 8 bytes](images/stack6.png)
...
for local variables](images/stack7.png)
...
![Next stack diagram, with the esp moved back up by 8 bytes](images/stack8.png)
...
its location](images/stack9.png)
...
its location](images/stack10.png)
...
In step 6, esp is moved down by 4 bytes. The number 4 is determined by the
compiler depending on the function being called. In this case, the compiler
decides 4 bytes are required to fit the local variable and any other data needed
...
[^4]:
    ...
    RISC stack design. For example, in RISC-V, `FP` is sometimes renamed `s0` and
...
[^6]:
    ...
    want to execute the instruction directly after the instruction eip is currently
    pointing to.
